...
result = get_base_data("SELECT * FROM `product_information` WHERE `marketplace`='ЯндексМаркет' ORDER BY RAND()")
...
for item in result:
    ...
    #     text_elem = get_by_css(item['link'], xpatch_dict['ОЗОН'], proxy)
    # if item['marketplace'] == "ЯндексМаркет":
    #     text_elem = get_by_css(item['link'], xpatch_dict['ЯндексМаркет'], proxy)
    #
